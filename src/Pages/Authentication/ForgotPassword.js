...
export default function ForgotPasswordComponent({ passwordPageScroll, nextPasswordPageFunction, closePasswordChangeFunc }){
	...
	async function handleSubmit(event) {
		...
		try {
			...
			const response = await fetch(`${server}/api/v1/user/${formState === 0 ? 'verifyEmail' : (formState === 1 ? 'verifyOTP' : (formState === 2 ? 'verifyAnswer' : 'setPassword'))}`, {
				method: "POST",
				headers: {
				...
				>
					<div className={classes.PopUpPage} onClick={(e) => e.stopPropagation()}>
						...
						<div className={classes.popUpInputDiv}>
						    <input ref={mailForm} className={classes.popupInput} placeholder="Enter your mail id..."></input>
					...
					<div className={classes.PopUpPage} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={`${classes.popUpInputBlock} ${classes.popUpInputBLockSecret}`}>
                            <label htmlFor="secretAnswerUpdatePassword">Secret Question : <p>{" " + secretQuestion}</p></label>
                            <div className={classes.popUpInputDiv}>
                                <input ref={secretAnsForm} className={classes.popupInput} id="secretAnswerUpdatePassword" placeholder="Enter the answer..."></input>
					...
					<div className={classes.PopUpPage} onClick={(e) => e.stopPropagation()}>						
						...
						<div className={classes.popUpInputBlock}>
							...
							<div className={classes.popUpInputDiv}>
								<input ref={passwordForm} className={classes.popupInput} id="passwordUpdatePassword" placeholder="Enter new password..."></input>
								<FontAwesomeIcon icon={faKey} />
							</div>							
						</div>
                        <div className={classes.popUpInputBlock}>
							...
							<div className={classes.popUpInputDiv}>
								<input ref={crfmPasswordForm} className={classes.popupInput} id="confirmPasswordUpdatePassword" placeholder="Retype your new password..."></input>
								<FontAwesomeIcon icon={faKey} />
							</div>							
						</div>
						<div className={classes.popUpButtonPlus}>
