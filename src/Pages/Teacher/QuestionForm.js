...
export default function QuestionForm() {
	...
	const testCasesAvailable = !queDetails
		? undefined
		: queDetails.testCase.map((item, idx) => ({
				id: idx + 1,
				testCase: item,
				answer: queDetails.answer[idx],
		  }));

	const [questionArray, setQuestionArray] = useState([]);
	const [difficulty, setDifficulty] = useState(
		queDetails ? queDetails.difficulty : ""
	);
	...
	async function handleSubmit(event) {
		...
		try {
			const response = await axios.post(
				`${server}/api/v1/question/${urlParam}`,
				newData
			);
				...
				<div className={classes.input}>
					...
					<div className={classes.inwrapper}>
						...
						<textarea
							...
							required
							defaultValue={
								queDetails ? queDetails.description : ""
							}
					...
					<div className={classes.inwrapper}>
						...
						<textarea
							...
							id="constraint"
							defaultValue={
								queDetails ? queDetails.constraints : ""
							}
							required
							className={classes.desc}
							style={{ height: "50px" }}
					...
					{testCases.map((testCase, index) => (
						<div key={testCase.id} className={classes.testcase}>
							...
							<input
								...
								onChange={(event) =>
									handleTestCaseChange(index, event)
								}
							/>
