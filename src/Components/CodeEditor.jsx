...
export default function CodeEditor({ testCase }) {
  ...
  const [stdin, setStdin] = useState(
    testCase?.reduce((final, val) => final + val + "\n", testCase.length + "\n")
  );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:4173/api/v1/question/submitCode/${params.questionId}`,
        {
          method: "POST",
          headers: {
  ...
  async function handleRun(event) {
    ...
    try {
      ...
      console.log(request);
      const response = await fetch(
        `http://localhost:4173/api/v1/question/runCode/${params.questionId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <Form onSubmit={handleSubmit} className={classes.wrapper}>
              ...
              <>
                {!response.output && (
                  <h2 className={classes.textAreaH2}>
                    {response.success ? "Submission Successful" : "Failed"}
                    <br />
                    {response.message}
                  </h2>
                )}
                {response.output && (
                  <h2 className={classes.textAreaH2}>OUTPUT</h2>
                )}
                {response.output && (
                  <p>
                    {response.output}
      ...
      <div className={classes.submission}>
        <button onClick={() => setCustomInput((prev) => !prev)} type="button">
